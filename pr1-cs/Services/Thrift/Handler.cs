...
using ParameterException = Model.Exceptions.Thrift.ParameterException;
using SignInException = Model.Exceptions.Thrift.SignInException;

namespace Services.Thrift
{
    public class Handler : Services.IAsync
    {
        ...
        public Handler(IUserRepository userRepository, IGameRepository gameRepository,
        ...
        public async Task<UserDto> signInUser(string username, string password, CancellationToken cancellationToken = default)
        {
            if (_signedInUsernames.Contains(username))
                throw new SignInException {Message = "user already signed in"};
            var user = _userRepository.GetOne(username);
            if (user.Password != password)
                throw new SignInException {Message = "incorrect password"};
        ...
        {
            if (!_signedInUsernames.Contains(username))
                throw new SignInException {Message = "user already signed out"};
            var user = _userRepository.GetOne(username);
        ...
        public async Task sellSeats(GameDto game, string clientName, int seatsCount, CancellationToken cancellationToken = default)
        {
            if (game.AvailableSeats < seatsCount)
                throw new ParameterException {Message = "not enough seats available"};
